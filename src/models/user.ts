...
export const findOne = async function (where: string): Promise<User | undefined> {
  const q = `SELECT provider, providerId, email, ffsToken FROM users WHERE ${where}`
...
export const findByProvider = async function (
  ...
  const q =
    "SELECT provider, providerId, email, ffsToken FROM users WHERE provider = ? and providerId = ?"
...
export const save = async function (user: User): Promise<void> {
  ...
  if (existingUser) {
    const sql = `
        ...
        `
    await db.run(sql, user.email, user.ffsToken, user.provider, user.providerId)
  } else {
    const sql = `
            INSERT INTO users(provider, providerId, email, ffsToken)
            VALUES
                (?, ?, ?, ?)
        `
    await db.run(sql, user.provider, user.providerId, user.email, user.ffsToken)
  }
}
